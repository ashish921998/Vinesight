...
export class AIService {
  ...
  static async getMockStreamingResponse(
    ...
    if (message.toLowerCase().includes('stream')) {
      mockResponse = "This is a mock streaming response to test the mobile UI. " + mockResponse + " The streaming animation should work smoothly on mobile devices with proper text wrapping and scroll behavior.";
    }
  ...
  private static generateFallbackResponse(message: string, context: any): string {
    ...
    if (conversationHistory.length > 0) {
      ...
      if (lastQuery && lastQuery.queryType === 'disease') {
        ...
        if (lowerMessage.includes('often') || lowerMessage.includes('frequency') || lowerMessage.includes('how many') || lowerMessage.includes('schedule')) {
      ...
      if (recentTopics.includes('irrigation') && (lowerMessage.includes('when') || lowerMessage.includes('time') || lowerMessage.includes('schedule'))) {
        return this.getIrrigationFollowUpResponse(lowerMessage, context);
      }
      
      if (recentTopics.includes('fertilizer') && (lowerMessage.includes('amount') || lowerMessage.includes('how much') || lowerMessage.includes('quantity'))) {
    ...
    const patterns = [
      {
        keywords: ['disease', 'sick', 'spots', 'fungus', 'mold', 'rot', 'infection', 'pathogen', 'बीमारी', 'रोग', 'आजार'],
        responses: {
      ...
      {
        keywords: ['water', 'irrigation', 'dry', 'drought', 'moisture', 'सिंचाई', 'पानी', 'सूखा', 'पाणी'],
        responses: {
      ...
      {
        keywords: ['fertilizer', 'nutrition', 'feed', 'nutrient', 'npk', 'उर्वरक', 'खाद', 'पोषण', 'खत'],
        responses: {
      ...
      {
        keywords: ['help', 'support', 'guide', 'how', 'what', 'when', 'why', 'मदद', 'सहायता', 'कैसे', 'क्या', 'कब'],
        responses: {
  ...
  private static getDiseaseResponse(message: string, context: any): string {
    ...
    let response = 'For disease identification, I recommend using our camera feature to take a clear photo of affected leaves or grape clusters. ';
    ...
    if (message.includes('powdery')) {
      response += 'Powdery mildew appears as white powdery growth. Apply sulfur-based fungicide and improve air circulation.';
    } else if (message.includes('downy')) {
      response += 'Downy mildew causes yellow spots and white downy growth. Use copper-based fungicide and ensure good drainage.';
    } else if (message.includes('black rot')) {
      response += 'Black rot causes black spots and mummified berries. Remove infected material and apply broad-spectrum fungicide.';
    } else {
      response += 'Common grape diseases include powdery mildew, downy mildew, botrytis, and black rot. Prevention through good canopy management is key.';
    }
  ...
  private static getIrrigationResponse(message: string, context: any): string {
    ...
    if (message.includes('how much') || message.includes('amount')) {
      response += 'Grapes typically need 24-36 inches of water annually. Use our ETc calculator for precise amounts based on your location and weather.';
      ...
      if (season === 'summer') {
        response += 'During summer, check soil moisture daily and irrigate when top 6 inches are dry.';
      } else if (season === 'spring') {
        response += 'In spring, maintain consistent moisture for new growth but avoid overwatering.';
      }
    } else {
      response += 'Monitor soil moisture at 6-12 inch depth. Maintain consistent moisture but avoid waterlogging. Reduce irrigation 2-3 weeks before harvest to concentrate sugars.';
    }
  ...
  private static getFertilizerResponse(message: string, context: any): string {
    ...
    if (message.includes('npk') || message.includes('ratio')) {
      ...
      response += 'Compost, aged manure, and cover crops provide slow-release nutrients. Apply 2-4 inches of compost annually.';
    } else if (message.includes('timing') || message.includes('when')) {
      if (season === 'spring') {
        response += 'Apply balanced fertilizer in early spring before bud break. Follow up with lighter applications through fruit set.';
      } else {
        response += 'Main fertilization in early spring, lighter applications during growing season, avoid late-season nitrogen to promote wood maturation.';
      }
    } else {
      response += 'Conduct soil tests to determine specific needs. Generally apply balanced NPK in spring, reduce nitrogen before harvest, and maintain adequate potassium for fruit quality.';
    }
  ...
  private static getHarvestResponse(message: string, context: any): string {
    ...
    if (message.includes('brix') || message.includes('sugar')) {
      response += 'Target Brix levels: 20-24° for table grapes, 22-26° for wine grapes. Test multiple clusters across the vineyard.';
    } else if (message.includes('color') || message.includes('appearance')) {
      response += 'Look for full color development, berry softening, and easy separation from stems. Seeds should be brown, not green.';
    } else if (message.includes('timing') || message.includes('when')) {
      response += 'Harvest in cool morning hours (6-10 AM) for best quality. Check sugar, acid levels, and taste regularly in final weeks.';
    } else {
      response += 'Monitor Brix levels, taste berries, check seed color (brown indicates maturity), and observe berry texture. Use our yield prediction feature for timing estimates.';
    }
  ...
  private static getPruningResponse(message: string, context: any): string {
    ...
    if (season === 'winter') {
      response += 'Winter is ideal for dormant pruning. Remove old, diseased wood and maintain 2-4 buds per spur for balanced production.';
    } else if (season === 'summer') {
      response += 'Summer pruning focuses on canopy management. Remove excess shoots, thin clusters if overcrowded, and maintain good air circulation.';
    } else {
      response += 'Prune during dormancy (late winter) for structure, and during growing season for canopy management. Remove diseased wood immediately regardless of season.';
    }
  ...
  private static getPestResponse(message: string, context: any): string {
    ...
    if (message.includes('aphid')) {
      response += 'Aphids can be controlled with beneficial insects (ladybugs), insecticidal soap, or targeted insecticides if severe.';
    } else if (message.includes('thrips')) {
      response += 'Thrips damage young shoots and leaves. Use blue sticky traps and beneficial predators. Avoid broad-spectrum insecticides.';
    } else if (message.includes('mite')) {
      response += 'Spider mites thrive in hot, dry conditions. Increase humidity, use predatory mites, or apply miticides if necessary.';
    } else {
      response += 'Implement IPM: regular monitoring, beneficial insects, targeted treatments only when needed. Common pests include aphids, thrips, leafhoppers, and spider mites.';
    }
  ...
  private static getWeatherResponse(message: string, context: any): string {
    ...
    if (message.includes('rain') || message.includes('wet')) {
      response += 'Excessive rain increases disease risk. Ensure good drainage, avoid spraying before rain, and monitor for fungal diseases closely.';
    } else if (message.includes('hot') || message.includes('heat')) {
      response += 'High temperatures stress vines. Increase irrigation frequency, provide shade cloth if extreme, and harvest earlier if needed.';
    } else if (message.includes('wind')) {
      response += 'Strong winds can damage vines and increase water stress. Install windbreaks and check trellis systems regularly.';
    } else {
      response += 'Monitor weather forecasts for spray timing, irrigation scheduling, and harvest planning. Extreme weather events require protective measures.';
    }
  ...
  private static getSoilResponse(message: string, context: any): string {
    ...
    if (message.includes('ph')) {
      response += 'Maintain soil pH between 6.0-7.5 for optimal nutrient availability. Test annually and adjust with lime or sulfur as needed.';
    } else if (message.includes('drainage')) {
      response += 'Grapes need well-draining soil. Install drainage systems in heavy clay soils or plant on slopes/raised beds.';
    } else if (message.includes('organic')) {
      response += 'Add organic matter (compost, aged manure) to improve soil structure, water retention, and microbial activity.';
    } else {
      response += 'Conduct annual soil tests for pH, nutrients, and organic matter. Grapes prefer well-draining, slightly acidic to neutral soils with good organic content.';
    }
  ...
  private static getGrapeResponse(message: string, context: any): string {
    ...
    if (message.includes('variety') || message.includes('cultivar')) {
      response += 'Choose varieties suited to your climate. Consider disease resistance, market demand, and intended use (table, wine, raisins).';
    } else if (message.includes('planting')) {
      response += 'Plant in well-prepared soil with good drainage. Space vines 6-10 feet apart in rows 8-12 feet apart depending on vigor and training system.';
    } else if (message.includes('training') || message.includes('trellis')) {
      response += 'Common training systems include VSP (Vertical Shoot Positioning) and GDC (Geneva Double Curtain). Choose based on variety and equipment.';
    } else {
      response += 'Successful grape growing requires proper site selection, variety choice, trellising system, regular pruning, disease management, and harvest timing.';
    }
  ...
  private static getDiseaseFollowUpResponse(type: string, context: any): string {
    ...
    switch (type) {
      ...
      case 'prevention':
        ...
        return preventionResponses[language as keyof typeof preventionResponses] || preventionResponses.en;
  ...
  static analyzeSoilData(soilData: {
    ...
    if (soilData.ph < 6.0 || soilData.ph > 7.5) {
      recommendations.push({
        ...
        actions: soilData.ph < 6.0 ? 
          ['Apply agricultural lime', 'Test pH monthly after application', 'Consider organic matter addition'] :
          ['Apply sulfur or acidifying fertilizer', 'Avoid alkaline fertilizers', 'Monitor pH regularly'],
