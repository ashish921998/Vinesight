...
export class SmartTaskGenerator {
   ...
   */
  static async generateSmartTasks(request: RecommendationRequest): Promise<AIServiceResponse<RecommendationResponse>> {
    try {
      ...
      const filteredTasks = taskRecommendations.filter(task => 
        !pendingTasks.some(pending => 
          pending.task_type === task.taskType && 
          Math.abs(new Date(pending.recommended_date).getTime() - task.recommendedDate.getTime()) < 24 * 60 * 60 * 1000
        )
      );
      ...
      return { 
        ...
        metadata: { 
          ...
          pestAlerts: pestPredictions.filter(p => p.riskLevel === 'high' || p.riskLevel === 'critical').length
        }
      };
  ...
  private static async generateTaskRecommendations(
    ...
    const irrigationTasks = this.generateIrrigationTasks(weather, farm, farmerProfile, context.recentTasks);
  ...
  private static generateIrrigationTasks(
    ...
    const daysSinceIrrigation = lastIrrigation ? 
      Math.floor((Date.now() - new Date(lastIrrigation.date).getTime()) / (1000 * 60 * 60 * 24)) : 7;
    ...
    if (dryDays >= 3 && daysSinceIrrigation >= 2) {
      ...
      const optimalDay = weather.find(day => 
  ...
  private static generateSprayTasks(
    ...
    criticalPests.forEach(pest => {
      ...
      const sprayWindow = weather.find(day => 
        day.windSpeed < 10 && 
        day.precipitation < 2 &&
      ...
      if (sprayWindow) {
        ...
        const treatment = pest.recommendedTreatments.chemical[0] || pest.recommendedTreatments.organic[0];
  ...
  private static generateHarvestTasks(context: TaskGenerationContext): AITaskRecommendation[] {
    ...
    if (farm.planting_date) {
      ...
      const now = new Date();
      const daysUntilHarvest = Math.floor((estimatedHarvestStart.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  ...
  private static generateFertigationTasks(context: TaskGenerationContext): AITaskRecommendation[] {
    ...
    const daysSinceFertigation = lastFertigation ? 
      Math.floor((Date.now() - new Date(lastFertigation.date).getTime()) / (1000 * 60 * 60 * 24)) : 30;
  ...
  private static generateMaintenanceTasks(context: TaskGenerationContext): AITaskRecommendation[] {
    ...
    if (currentMonth >= 11 || currentMonth <= 2) { // Nov-Feb pruning season
      ...
      if (!lastPruning || new Date(lastPruning.date).getFullYear() < new Date().getFullYear()) {
        tasks.push({
          ...
          priorityScore: 0.6,
          weatherDependent: false,
          reasoning: 'Dormant season pruning to maintain vine health and optimize next season\'s yield.',
          confidenceScore: 0.9,
          status: 'pending',
          farmerFeedback: undefined,
          outcomeTracked: false,
  ...
  private static generateSoilTestTasks(context: TaskGenerationContext): AITaskRecommendation[] {
    ...
    const monthsSinceSoilTest = lastSoilTest ? 
      Math.floor((Date.now() - new Date(lastSoilTest.date).getTime()) / (1000 * 60 * 60 * 24 * 30)) : 12;
   ...
   * Helper methods
   */
  private static async getWeatherForecast(latitude: number, longitude: number): Promise<WeatherData[] | null> {
    try {
  ...
  private static async getRecentActivities(farmId: number): Promise<any[]> {
    try {
      ...
      const [irrigations, sprays, fertigations] = await Promise.all([
        supabase.from('irrigation_records').select('*').eq('farm_id', farmId).gte('date', thirtyDaysAgo.toISOString().split('T')[0]),
        supabase.from('spray_records').select('*').eq('farm_id', farmId).gte('date', thirtyDaysAgo.toISOString().split('T')[0]),
        supabase.from('fertigation_records').select('*').eq('farm_id', farmId).gte('date', thirtyDaysAgo.toISOString().split('T')[0])
      ]);
  ...
  private static calculateConsecutiveDryDays(weather: WeatherData[]): number {
    ...
    weather.forEach(day => {
      if (day.precipitation < 2) { // <2mm considered dry
        currentDryStreak++;
  ...
  private static recommendFertilizer(growthStage: string): { type: string; alternative: string } {
    ...
    return recommendations[growthStage as keyof typeof recommendations] || recommendations['Budbreak'];
  }
  ...
  private static generateInsights(context: TaskGenerationContext, pestPredictions: any[]): any[] {
    ...
    // Pest insights
    const highRiskPests = pestPredictions.filter(p => p.riskLevel === 'high' || p.riskLevel === 'critical');
  ...
  private static generateRiskAssessment(context: TaskGenerationContext, pestPredictions: any[]): any {
    ...
    const pestRisk = pestPredictions.length > 0 ? 
      Math.max(...pestPredictions.map(p => p.probabilityScore)) : 0.2;
  ...
  private static generatePersonalizedNotes(farmerProfile: FarmerAIProfile, context: TaskGenerationContext): string[] {
    const notes = [];
    ...
    if (farmerProfile.successMetrics.averageYield > 0) {
      notes.push(`Your average yield performance: ${farmerProfile.successMetrics.averageYield.toFixed(1)} kg/acre`);
    }
