...
export class LAICalculator {
  ...
  private static generateRecommendations(
    ...
    return {
      canopyManagement,
      pruningAdvice,
      trellisAdjustments: trellisAdjustments.length > 0 ? trellisAdjustments : ['Current trellis system is appropriate']
    };
  }
   ...
   */
  private static assessQualityMetrics(lai: number, lightInterception: number): LAIResults['qualityMetrics'] {
  ...
  static getOptimalLAITargets(productionGoal: 'table' | 'wine' | 'raisin'): {
    ...
    const targets = {
      table: {
        ...
        reasoning: 'Table grapes need excellent fruit exposure for color and size while maintaining adequate leaf area for photosynthesis'
      },
      wine: {
        minLAI: 2.2,
        maxLAI: 3.5,
        optimalRange: '2.5 - 3.0',
        reasoning: 'Wine grapes benefit from moderate shading for flavor development while ensuring sufficient photosynthetic capacity'
      },
      raisin: {
        minLAI: 2.0,
        maxLAI: 3.2,
        optimalRange: '2.3 - 2.8',
        reasoning: 'Raisin grapes require balanced canopy for sugar accumulation and efficient drying conditions'
      }
    };
