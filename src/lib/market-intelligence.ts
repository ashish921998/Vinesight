...
export class MarketIntelligenceService {
  ...
  ): { type: 'price' | 'demand' | 'supply' | 'opportunity'; message: string; priority: 'high' | 'medium' | 'low' }[] {
    const alerts = [];
  ...
  private static generateSampleMarketData(): MarketData[] {
    ...
    for (let i = 0; i < 90; i++) { // 3 months of daily data
      const date = new Date();
  ...
  private static analyzeHistoricalPrices(variety: string, quality: string) {
    const relevantData = this.marketData.filter(d => 
      d.grapeVariety === variety && d.quality === quality
    );
  ...
  private static async assessWeatherImpact(harvestDate: Date) {
    ...
    return {
      impact: impactScore,
      description: impactScore > 0.1 ? 'Favorable weather conditions' :
                   impactScore < -0.1 ? 'Challenging weather conditions' :
