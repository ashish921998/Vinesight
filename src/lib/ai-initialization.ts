...
export class AIInitializationService {
   ...
   */
  static async initializeAIForFarm(farmId: number, userId: string): Promise<{
    success: boolean;
    predictions?: any[];
    recommendations?: any[];
    profile?: any;
    ...
    try {
      ...
      return {
        ...
        recommendations: recommendationResult.success ? recommendationResult.data?.taskRecommendations : [],
        profile
      };
   ...
   */
  static async testAIServices(farmId: number, userId: string): Promise<{
    success: boolean;
    ...
    try {
      ...
      results.pestPredictions = {
        ...
        highRiskAlerts: activePredictions.filter(p => p.riskLevel === 'high' || p.riskLevel === 'critical').length
      };
   ...
   */
  static async seedDemoData(farmId: number, userId: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const demoPrediction = {
        ...
        predicted_onset_date: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        weather_triggers: {
        ...
        recommended_treatments: {
          ...
          organic: [
            { method: 'Bordeaux mixture', description: 'Copper-based fungicide', effectiveness: 0.7 }
          ],
      ...
      const demoTask = {
        ...
        reasoning: 'Demo: High temperature forecast with low humidity requires irrigation attention',
