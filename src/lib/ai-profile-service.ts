...
export class AIProfileService {
  ...
  static async updateProfileFromDecision(
    ...
    try {
      ...
      if (outcome.yield !== undefined) {
        updatedMetrics.averageYield =
          updatedMetrics.averageYield * (1 - alpha) + outcome.yield * alpha
      }

      if (outcome.costs !== undefined && outcome.yield !== undefined) {
        const currentEfficiency = outcome.yield / outcome.costs
        updatedMetrics.costEfficiency =
          updatedMetrics.costEfficiency * (1 - alpha) + currentEfficiency * alpha
      }
  ...
  static calculateFarmerSimilarity(profile1: FarmerAIProfile, profile2: FarmerAIProfile): number {
    ...
    const adoptionScore =
      profile1.decisionPatterns.adoptionSpeed === profile2.decisionPatterns.adoptionSpeed ? 1 : 0.5
    ...
    const commScore =
      profile1.decisionPatterns.communicationStyle === profile2.decisionPatterns.communicationStyle
        ? 1
        : 0.7
  ...
  static async getAnonymizedProfiles(region?: string): Promise<any[]> {
    try {
      let query = supabase.from('farmer_ai_profiles').select(`
          risk_tolerance,
          decision_patterns,
          success_metrics,
