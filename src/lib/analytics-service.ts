...
export class AnalyticsService {
  ...
  private static scoreIrrigationPractice(avgDuration: number, frequency: number): number {
    ...
    if (frequency >= 12 && frequency <= 16)
      score += 15 // 3-4 times per week
  ...
  private static scoreYieldQuality(harvests: any[], area: number): number {
    ...
    if (yieldPerHa >= 15)
      score += 25 // Excellent yield
    else if (yieldPerHa >= 12)
      score += 15 // Good yield
    ...
    const avgGradeScore =
      harvests.reduce((sum, h) => sum + (gradeScores[h.grade] || 0), 0) / harvests.length
    score += avgGradeScore
