...
export function validateEToCalculation(testData = FAO56_EXAMPLE_DATA) {
  ...
  console.log('Percentage Error:', (((result.eto - testData.expectedETo) / testData.expectedETo) * 100).toFixed(1) + '%');
  console.log('============================');
...
export function testFylloData() {
  ...
  console.log('Percentage Error:', (((result.eto - FYLLO_TEST_DATA.expectedETo) / FYLLO_TEST_DATA.expectedETo) * 100).toFixed(1) + '%');
  
  return {
    expected: FYLLO_TEST_DATA.expectedETo,
    calculated: result.eto,
    difference: result.eto - FYLLO_TEST_DATA.expectedETo,
    percentageError: ((result.eto - FYLLO_TEST_DATA.expectedETo) / FYLLO_TEST_DATA.expectedETo) * 100
  };
}