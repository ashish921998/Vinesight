...
export class IoTSensorService {
  ...
  static getSensorAnalytics(sensorId: string, timeRange: '24h' | '7d' | '30d' | '90d' = '7d'): SensorAnalytics {
  ...
  private static checkThresholds(sensor: SensorDevice, reading: SensorReading) {
    ...
    if (reading.value < sensor.thresholds.min || reading.value > sensor.thresholds.max) {
      const alert: SensorAlert = {
        ...
        sensorId: sensor.id,
        alertType: 'threshold',
        severity: reading.value < sensor.thresholds.min * 0.5 || reading.value > sensor.thresholds.max * 1.5 ? 'critical' : 'high',
  ...
  private static averageCurrentValue(sensors: SensorDevice[], readings: { [key: string]: SensorReading }): number {
    const values = sensors.map(sensor => readings[sensor.id]?.value).filter(v => v !== undefined);
