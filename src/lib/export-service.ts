...
export class ExportService {
  ...
  private static async exportToCSV(data: ExportData, options: ExportOptions): Promise<void> {
    ...
    if (reportType === 'operations' || reportType === 'comprehensive') {
      ...
      if (data.irrigation.length > 0) {
        ...
        csvContent +=
          'Date,Duration (hrs),Area (ha),Growth Stage,Moisture Status,System Discharge (L/h),Notes\n'
      ...
      if (data.fertigation.length > 0) {
        ...
        csvContent +=
          'Date,Fertilizer Type,Quantity (kg),Area (ha),Duration (hrs),Concentration,pH,EC,Notes\n'
      ...
      if (data.harvest.length > 0) {
        ...
        data.harvest.forEach((record) => {
          const totalValue = record.price_per_kg
            ? (record.quantity * record.price_per_kg).toFixed(2)
            : ''
  ...
  static async getAvailableExportTypes(farmId: number): Promise<string[]> {
    ...
    try {
      const [irrigation, spray, fertigation, harvest, expenses, calculations, soilTests, tasks] =
        await Promise.all([
