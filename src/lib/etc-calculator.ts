...
const GRAPE_KC_VALUES: Record<GrapeGrowthStage, { kc: number; description: string }> = {
  ...
  budbreak: { kc: 0.3, description: 'Early season - buds swelling and breaking' },
  flowering: { kc: 0.7, description: 'Flowering stage - moderate water needs' },
...
export class ETcCalculator {
  ...
  static calculateETo(weather: WeatherData, location: ETcCalculationInputs['location']): number {
    ...
    const numerator =
      0.408 * delta * (rn - G) + ((gamma * 900) / (tmean + 273)) * windSpeed * (es - ea)
  ...
  private static calculateNetRadiation(
    ...
    if (solarRadiation !== undefined) {
      ...
      Rs = (0.25 + (0.5 * sunshineHours) / maxSunshineHours) * Ra
    } else {
    ...
    const Rnl =
      ((stefanBoltzmann *
        (Math.pow(temperatureMax + 273.16, 4) + Math.pow(temperatureMin + 273.16, 4))) /
        2) *
  ...
  private static validateWeatherData(weather: WeatherData): void {
    ...
    if (
      weather.solarRadiation !== undefined &&
      (weather.solarRadiation < 0 || weather.solarRadiation > 45)
    ) {
    ...
    if (
      weather.solarRadiationLux !== undefined &&
      (weather.solarRadiationLux < 0 || weather.solarRadiationLux > 150000)
    ) {
    ...
    if (
      weather.sunshineHours !== undefined &&
      (weather.sunshineHours < 0 || weather.sunshineHours > 16)
    ) {
