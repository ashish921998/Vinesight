export interface SoilTestData {
  ...
  // Physical Properties
  physical: {
    soilTexture: 'sand' | 'loamy_sand' | 'sandy_loam' | 'loam' | 'silt_loam' | 'silt' | 'clay_loam' | 'silty_clay_loam' | 'sandy_clay' | 'silty_clay' | 'clay';
...
export class SoilHealthAnalyzer {
  static analyzeSoilHealth(inputs: SoilHealthInputs): SoilHealthResults {
    ...
    return {
      healthMetrics: {
        ...
        category,
        subscores: {
          physical: { score: Math.round(physicalScore), indicators: this.getPhysicalIndicators(testData.physical) },
          chemical: { score: Math.round(chemicalScore), indicators: this.getChemicalIndicators(testData.chemical) },
          biological: { score: Math.round(biologicalScore), indicators: this.getBiologicalIndicators(testData.biological) }
        },
        limitations,
        trends
      },
  ...
  private static calculatePhysicalScore(physical: SoilTestData['physical']): number {
    ...
    if (physical.bulkDensity >= standards.bulkDensity.min && physical.bulkDensity <= standards.bulkDensity.max) {
    ...
    // Porosity score
    if (physical.porosity >= standards.porosity.min && physical.porosity <= standards.porosity.max) {
    ...
    if (physical.waterHoldingCapacity >= standards.waterHoldingCapacity.min && physical.waterHoldingCapacity <= standards.waterHoldingCapacity.max) {
      const optimalDistance = Math.abs(physical.waterHoldingCapacity - standards.waterHoldingCapacity.optimal);
    ...
    const textureScores = {
      'sand': 40, 'loamy_sand': 60, 'sandy_loam': 85, 'loam': 100, 'silt_loam': 90,
      'silt': 70, 'clay_loam': 75, 'silty_clay_loam': 70, 'sandy_clay': 50, 'silty_clay': 45, 'clay': 40
    };
  ...
  private static calculateChemicalScore(chemical: SoilTestData['chemical']): number {
    ...
    const pHScore = this.calculateParameterScore(chemical.pH, standards.pH.min, standards.pH.max, standards.pH.optimal);
    score += pHScore * 2; // Weight pH heavily
    components += 2;
    
    // Organic matter score (critical for soil health)
    const omScore = this.calculateParameterScore(chemical.organicMatter, standards.organicMatter.min, standards.organicMatter.max, standards.organicMatter.optimal);
    ...
    score += this.calculateParameterScore(chemical.nitrogen.available, standards.nitrogen.min, standards.nitrogen.max, standards.nitrogen.optimal);
    score += this.calculateParameterScore(chemical.phosphorus.available, standards.phosphorus.min, standards.phosphorus.max, standards.phosphorus.optimal);
    score += this.calculateParameterScore(chemical.potassium.available, standards.potassium.min, standards.potassium.max, standards.potassium.optimal);
    components += 3;
    
    // Secondary nutrient scores
    score += this.calculateParameterScore(chemical.calcium, standards.calcium.min, standards.calcium.max, standards.calcium.optimal) * 0.5;
    score += this.calculateParameterScore(chemical.magnesium, standards.magnesium.min, standards.magnesium.max, standards.magnesium.optimal) * 0.5;
    score += this.calculateParameterScore(chemical.sulfur, standards.sulfur.min, standards.sulfur.max, standards.sulfur.optimal) * 0.5;
    components += 1.5;
  ...
  private static calculateParameterScore(value: number, min: number, max: number, optimal: number): number {
  ...
  private static identifyLimitations(testData: SoilTestData, standards: any): SoilHealthMetrics['limitations'] {
  ...
  private static generateAlerts(testData: SoilTestData, standards: any): SoilHealthResults['alerts'] {
  ...
  private static generateRecommendations(testData: SoilTestData, farmContext: any, limitations: any): SoilRecommendations {
    ...
    seasonal.push(
      ...
      },
      {
        ...
        materials: [
          { name: 'Cover crop seeds', quantity: '20-30 kg/ha', cost: 3000 }
        ]
      },
      {
        season: 'post_monsoon',
        ...
        materials: [
          { name: 'Micronutrient mix', quantity: '25 kg/ha', cost: 5000 }
        ]
      },
      {
        season: 'winter',
        ...
        materials: [
          { name: 'Farm yard manure', quantity: '10-15 t/ha', cost: 20000 }
        ]
      }
    );
    ...
    if (testData.chemical.nitrogen.available < 200) {
      fertilizer.push({
        ...
        recommendation: {
          organic: { source: 'Compost + Green manure', quantity: '8-10 t/ha', timing: 'Pre-monsoon' },
  ...
  private static calculateProjections(testData: SoilTestData, recommendations: SoilRecommendations): SoilHealthResults['projections'] {
  ...
  private static generateIntegrations(testData: SoilTestData, overallScore: number): SoilHealthResults['integrations'] {
    return {
      nutrientCalculator: {
  ...
  private static calculateConfidence(testData: SoilTestData): number {
    ...
    if (!testData.chemical.pH || testData.chemical.pH < 4 || testData.chemical.pH > 10) confidence -= 15;
