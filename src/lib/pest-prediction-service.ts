...
export class PestPredictionService {
  ...
  private static readonly PEST_RISK_FACTORS: PestRiskFactors = {
    downy_mildew: {
      ...
      seasonalFactor: {
        'january': 0.2, 'february': 0.3, 'march': 0.5, 'april': 0.7,
        'may': 0.9, 'june': 1.0, 'july': 0.8, 'august': 0.7,
        'september': 0.5, 'october': 0.3, 'november': 0.2, 'december': 0.1
      }
    },
    powdery_mildew: {
      ...
      seasonalFactor: {
        'january': 0.1, 'february': 0.2, 'march': 0.4, 'april': 0.6,
        'may': 0.8, 'june': 0.9, 'july': 1.0, 'august': 0.9,
        'september': 0.7, 'october': 0.5, 'november': 0.3, 'december': 0.1
      }
    },
    black_rot: {
      ...
      seasonalFactor: {
        'january': 0.1, 'february': 0.1, 'march': 0.3, 'april': 0.5,
        'may': 0.7, 'june': 0.9, 'july': 1.0, 'august': 0.8,
        'september': 0.6, 'october': 0.4, 'november': 0.2, 'december': 0.1
      }
    },
    anthracnose: {
      ...
      seasonalFactor: {
        'january': 0.2, 'february': 0.3, 'march': 0.5, 'april': 0.7,
        'may': 0.8, 'june': 1.0, 'july': 0.9, 'august': 0.8,
        'september': 0.6, 'october': 0.4, 'november': 0.2, 'december': 0.1
      }
    },
    thrips: {
      ...
      historicalMultiplier: 1.0,
      seasonalFactor: {
        'january': 0.3, 'february': 0.4, 'march': 0.6, 'april': 0.8,
        'may': 1.0, 'june': 0.9, 'july': 0.8, 'august': 0.7,
        'september': 0.6, 'october': 0.5, 'november': 0.4, 'december': 0.3
      }
    }
  };
  ...
  private static readonly TREATMENT_RECOMMENDATIONS = {
    downy_mildew: {
      ...
      organic: [
        { method: 'Bordeaux mixture', description: 'Copper-based fungicide spray', effectiveness: 0.7 },
        { method: 'Baking soda spray', description: 'Potassium bicarbonate solution', effectiveness: 0.6 },
        { method: 'Neem oil', description: 'Organic oil spray with fungicidal properties', effectiveness: 0.65 }
      ],
      cultural: [
    ...
    powdery_mildew: {
      ...
      organic: [
        ...
        { method: 'Bicarbonate spray', description: 'Sodium/Potassium bicarbonate', effectiveness: 0.65 }
      ],
      cultural: [
    ...
    black_rot: {
      ...
      organic: [
        ...
        { method: 'Compost tea', description: 'Beneficial microorganism spray', effectiveness: 0.5 }
      ],
      cultural: [
    ...
    thrips: {
      ...
      organic: [
        ...
        { method: 'Insecticidal soap', description: 'Soap-based spray solution', effectiveness: 0.6 },
        { method: 'Blue sticky traps', description: 'Physical trapping method', effectiveness: 0.5 }
      ],
      cultural: [
  ...
  static async generatePredictions(
    ...
    try {
      ...
      for (const [pestType, factors] of Object.entries(this.PEST_RISK_FACTORS)) {
        ...
        if (riskScore > 0.3) { // Only create predictions for significant risk
  ...
  private static calculatePestRisk(
    ...
    // Humidity risk
    if (factors.humidity) {
      const avgHumidity = weatherData.reduce((sum, day) => sum + day.humidity.avg, 0) / weatherData.length;
      const humidityRisk = avgHumidity >= factors.humidity.threshold ? 1.0 : avgHumidity / factors.humidity.threshold;
    ...
    // Rainfall risk
    if (factors.rainfall) {
      const rainyDays = weatherData.filter(day => day.precipitation >= factors.rainfall.threshold).length;
      const rainfallRisk = rainyDays >= factors.rainfall.days ? 1.0 : rainyDays / factors.rainfall.days;
    ...
    if (factors.drySpell) {
      ...
      const drySpellRisk = maxDrySpell >= factors.drySpell.days ? 1.0 : maxDrySpell / factors.drySpell.days;
    ...
    if (factors.windSpeed) {
      const avgWindSpeed = weatherData.reduce((sum, day) => sum + day.windSpeed, 0) / weatherData.length;
      const windRisk = avgWindSpeed <= factors.windSpeed.threshold ? 1.0 : 
  ...
  private static calculateOnsetDate(weatherData: WeatherData[], factors: any): Date {
    ...
    weatherData.forEach((day, index) => {
      ...
      if (factors.temperature) {
        const [minOptimal, maxOptimal] = factors.temperature.optimal;
        const tempRisk = (day.temperature.avg >= minOptimal && day.temperature.avg <= maxOptimal) ? 1.0 : 0.5;
