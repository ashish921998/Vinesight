...
export class OpenMeteoGeocodingService {
   ...
   */
  static async getLocationSuggestions(query: string, maxResults: number = 5): Promise<LocationResult[]> {
    if (query.length < 2) {
      return [];
    }
    ...
    try {
      ...
      return results
        ...
        .sort((a, b) => {
          ...
          return (importance[b.feature_code as keyof typeof importance] || 0) - 
  ...
  static async reverseGeocode(latitude: number, longitude: number): Promise<LocationResult | null> {
    try {
      ...
      if (results.length > 0) {
        ...
        const distance = this.calculateDistance(latitude, longitude, result.latitude, result.longitude);
