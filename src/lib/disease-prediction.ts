...
export class DiseasePredictionModel {
  ...
  private static calculateDiseaseRisk(
    ...
    if (model.rainRequirement && totalRainfall > 5)
      factors.push(`Recent rainfall (${totalRainfall}mm)`)
    if (maxLeafWetness > model.leafWetnessThreshold)
      factors.push(`Extended leaf wetness (${maxLeafWetness}h)`)
  ...
  private static calculateConfidence(weatherData: WeatherData[]): number {
    ...
    const dataAge =
      weatherData.length > 0
        ? (Date.now() - weatherData[weatherData.length - 1].date.getTime()) / (1000 * 60 * 60 * 24)
        : 7
