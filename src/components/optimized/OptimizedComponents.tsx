...
export const OptimizedFinancialSummary = memo<FinancialSummaryProps>(
  ({ totalRevenue, totalCosts, netProfit, profitMargin, period }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div
                className={`h-10 w-10 rounded-lg ${profitStatus.bg} flex items-center justify-center`}
              >
                <ProfitIcon className={`h-5 w-5 ${profitStatus.color}`} />
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Net Profit</p>
                <p className={`text-lg font-bold ${profitStatus.color}`}>{formatCurrency.profit}</p>
              </div>
            </div>
          </CardContent>
        </Card>
...
export const OptimizedDataTable = memo<OptimizedDataTableProps>(
  ({ data, onItemClick, sortBy = 'name', filterCategory }) => {
    ...
    const TableRow = memo<{ item: DataItem }>(({ item }) => (
      ...
      >
        ...
        <td className="px-4 py-2 text-muted-foreground text-sm">{formatDate(item.date)}</td>
      </tr>
    ))
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            {filterCategory && <Badge variant="secondary">{filterCategory}</Badge>}
          </div>
        </CardHeader>
        <CardContent>
...
export const OptimizedChartContainer = memo<OptimizedChartContainerProps>(
  ({ title, data, chartType, height = 300, showLegend = true }) => {
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">{chartData.length} data points</span>
          </div>
        </CardHeader>
        <CardContent>
          <div
