...
export function TodaysTasksSection({ 
  ...
  const getTaskIcon = (type: string) => {
    switch (type) {
      case 'irrigation': return <Droplets className="h-4 w-4 text-primary" />;
      case 'spray': return <SprayCan className="h-4 w-4 text-primary" />;
      case 'harvest': return <Scissors className="h-4 w-4 text-primary" />;
      ...
      <CardHeader className="pb-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            {onAddTask && (
              <Button size="sm" variant="outline" onClick={onAddTask} className="h-9 px-3 text-xs touch-manipulation rounded-full">
          ...
          {(pendingTasks.length > 0 || overdueTasks.length > 0) && (
            <div className="flex gap-2">
              {pendingTasks.length > 0 && (
                <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 text-xs px-2">
                  {pendingTasks.length} pending
                </Badge>
              )}
              {overdueTasks.length > 0 && (
                <Badge variant="outline" className="bg-red-100 text-red-800 border-red-200 text-xs px-2">
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {isOverdue && (
                          <Badge variant="outline" className="text-xs bg-red-100 text-red-800 border-red-200 px-2 py-0.5">
                            overdue
                          </Badge>
                        )}
                      </div>
                  ...
                  <div className="ml-11 space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      {task.estimatedDuration && (
                        <span className="text-muted-foreground">~{formatDuration(task.estimatedDuration)}</span>
                      )}
                    </div>
              ...
              <div className="pt-4 border-t border-border">
                ...
                {completedTasks.length > 2 && (
                  <div className="text-center mt-3">
                    <Button variant="ghost" size="sm" className="text-sm text-muted-foreground h-9 px-4 touch-manipulation">
