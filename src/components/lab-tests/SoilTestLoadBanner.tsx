...
export function SoilTestLoadBanner({ farmId, onLoadTest }: SoilTestLoadBannerProps) {
  ...
  return (
    <Alert className={`${isOld ? 'bg-yellow-50 border-yellow-300' : 'bg-blue-50 border-blue-300'} mb-6`}>
      <FlaskConical className={`h-4 w-4 ${isOld ? 'text-yellow-600' : 'text-blue-600'}`} />
      <AlertDescription className="flex items-start justify-between gap-4">
        <div className="flex-1 space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-semibold text-foreground">Soil test available from your farm</span>
            ...
            {isOld && (
              <Badge variant="outline" className="text-xs bg-yellow-100 border-yellow-400 text-yellow-700">
                ⚠️ Consider new test
              </Badge>
            )}
          </div>
          <p className="text-sm text-muted-foreground">
            Load test results to pre-fill soil parameters (pH, EC, N, P, K, micronutrients) instead of entering manually.
            {isOld && ' Note: This test is over 4 months old. Consider taking a new test for accurate recommendations.'}
          </p>
