...
export default function FarmEfficiencyPage() {
  ...
  const loadFarmEfficiencyMetrics = async () => {
    ...
    try {
      ...
      const totalRevenue = harvestRecords.reduce((sum, record) => sum + (record.quantity * (record.price || 0)), 0);
      ...
      const efficiencyMetrics: FarmEfficiencyMetric[] = [
        {
          ...
          unit: 'kg/ha',
          status: yieldPerHectare > 3000 ? 'excellent' : yieldPerHectare > 2000 ? 'good' : yieldPerHectare > 1000 ? 'needs-improvement' : 'poor',
          benchmark: 3000,
          category: 'yield'
        },
        {
          ...
          unit: 'kg/L',
          status: waterEfficiency > 0.8 ? 'excellent' : waterEfficiency > 0.5 ? 'good' : waterEfficiency > 0.3 ? 'needs-improvement' : 'poor',
          benchmark: 0.8,
          category: 'water'
        },
        {
          ...
          unit: '₹/kg',
          status: avgPrice > 80 ? 'excellent' : avgPrice > 60 ? 'good' : avgPrice > 40 ? 'needs-improvement' : 'poor',
          benchmark: 80,
          category: 'cost'
        },
        {
          ...
          unit: '₹000/ha',
          status: revenuePerHectare > 200000 ? 'excellent' : revenuePerHectare > 120000 ? 'good' : revenuePerHectare > 60000 ? 'needs-improvement' : 'poor',
          benchmark: 200,
          category: 'cost'
        },
        {
          ...
          unit: 'harvests',
          status: harvestRecords.length >= 3 ? 'excellent' : harvestRecords.length >= 2 ? 'good' : harvestRecords.length >= 1 ? 'needs-improvement' : 'poor',
          benchmark: 3,
          category: 'quality'
        },
        {
          ...
          unit: 'treatments',
          status: sprayRecords.length <= 8 && sprayRecords.length >= 4 ? 'excellent' : sprayRecords.length <= 12 ? 'good' : 'needs-improvement',
          benchmark: 8,
          category: 'quality'
        },
        {
          ...
          unit: 'L/ha (000)',
          status: (totalIrrigation / selectedFarm.area) < 500000 ? 'excellent' : (totalIrrigation / selectedFarm.area) < 800000 ? 'good' : 'needs-improvement',
          benchmark: 500,
          category: 'water'
        },
        {
      ...
      const defaultMetrics: FarmEfficiencyMetric[] = [
        ...
        { name: 'Water Use Efficiency', value: 0.65, unit: 'kg/L', status: 'good', benchmark: 0.8, category: 'water' },
        { name: 'Average Grape Price', value: 72, unit: '₹/kg', status: 'good', benchmark: 80, category: 'cost' },
        { name: 'Revenue per Hectare', value: 180, unit: '₹000/ha', status: 'good', benchmark: 200, category: 'cost' }
      ];
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'yield': return <Grape className="h-4 w-4" />;
      case 'water': return <Droplets className="h-4 w-4" />;
      case 'cost': return <DollarSign className="h-4 w-4" />;
      case 'quality': return <Leaf className="h-4 w-4" />;
      ...
      {!selectedFarm ? (
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Based on yield, water efficiency, cost management, and quality metrics for {selectedFarm.name}
            </CardDescription>
          </CardHeader>
        <CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {metrics.filter(metric => 
                ['Yield per Hectare', 'Water Use Efficiency', 'Average Grape Price', 'Revenue per Hectare']
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Water usage and irrigation optimization metrics
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Production output and quality management metrics
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {metrics.filter(metric => 
                  ['Yield per Hectare', 'Harvest Frequency', 'Spray Applications', 'Farm Utilization'].includes(metric.name)
                ).map((metric, index) => (
                <div key={index} className="flex items-center justify-between p-3 rounded-lg border">
                  <div className="flex items-center gap-3">
                    {getStatusIcon(metric.status)}
